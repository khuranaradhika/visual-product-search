...
aws s3 cp s3://amazon-berkeley-objects/listings/metadata/ \
    "$DATA_DIR/listings/metadata/" \
    --recursive --no-sign-request

echo ""
echo "=== 2/3  Downloading images metadata CSV ==="
aws s3 cp s3://amazon-berkeley-objects/images/metadata/ \
    "$DATA_DIR/images/metadata/" \
    --recursive --no-sign-request

echo ""
echo "=== 3/3  Downloading catalog images (~40 GB total) ==="
echo "    This is large! You can Ctrl-C and use --max-items in training instead."
echo "    Or download a subset with:  aws s3 cp ... --exclude '*' --include '0/*' "
echo ""
aws s3 cp s3://amazon-berkeley-objects/images/original/ \
